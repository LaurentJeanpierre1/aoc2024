class Day21Cache {
    ...
    private val cache = Array(26) { mutableMapOf<Context, Long>() }
    ...
    fun typePad(code: String, depth: Int, fromKey: Char) : Long {
    ...
    fun compute(seq: String, depth: Int) : Long = if (depth==0) seq.length.toLong() else {
        var prev = 'A'
        var total = 0L
        for (current in seq) {
            val context = Context(prev, current)

            total += cache[depth].computeIfAbsent(context) {
                _ -> typePad(current.toString(), depth-1, prev)
            }
            prev = current
        }
        total
